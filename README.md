...
基于 ESP32-C3 的四个嵌入式课程设计项目，外加一个集大成的综合项目（电子宠物），全部采用 Arduino 框架开发，**不使用任何第三方库**，所有外设驱动均为底层手写实现。
...
集成前四个项目几乎全部外设的养成类电子宠物，三层架构（驱动层 → 应用层 → 主程序），13 状态非阻塞状态机驱动，配套手机 Web 作弊控制台。

- **显示**: SSD1315 OLED (I2C, UI 渲染) + SSD1680 墨水屏 (双色宠物表情)
- **传感器**: BH1750 光照 (昼夜判断) + DHT11 温湿度 + MPU6050 加速度计 (摇晃互动)
- **输入**: TTP229-LSF 16 键触摸键盘 (I2C)
- **时钟**: DS1307 RTC (I2C) + WiFi NTP 校时
- **反馈**: 无源蜂鸣器 (LEDC PWM, 按键音阶)
- **玩法**: 喂食 / 抚摸 / 摇晃 / 逗乐小游戏 / 打工 / 战斗，HP·心情·金币自然衰减 (夜间减半)
- **存档**: ESP32 NVS A/B 双槽交替写入 + XOR 校验 + 离线衰减恢复
- **联网**: 手机浏览器访问内置 HTTP Server (端口 80) 实时修改宠物属性
